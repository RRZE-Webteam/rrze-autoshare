    ...
    return (
        <span>
            ...
            {errorMessage?.includes(
                ...
                >
                    {__("Learn more here.", "rrze-autoshare")}
                </ExternalLink>
            )}
        </span>
    );
...
export function StatusLogs({ messages }) {
    ...
    return (
        <div className="rrze-autoshare-twitter-post-status">
            {messages.message.map((statusMessage, index) => {
                const TweetIcon = getIconByStatus(statusMessage.status);
                return (
                    <div className="rrze-autoshare-twitter-log" key={index}>
                        {TweetIcon}
                        <span>
                            {statusMessage.url ? (
                            ...
                            {!!statusMessage.handle && (
                                <strong>{` - @` + statusMessage.handle}</strong>
                            )}
                        </span>
                    </div>
                );
