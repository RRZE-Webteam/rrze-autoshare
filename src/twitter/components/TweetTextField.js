...
const { siteUrl, isLocalSite, twitterURLLength } = adminAutoshareTwitter;
...
export function TweetTextField() {
    ...
    const getTweetLength = () => {
        ...
        if (240 <= length) {
            return {
                tweetLength: sprintf(
                    /* translators: %d is tweet message character count */
                    __("%d - Getting Long!", "rrze-autoshare"),
                    length
                ),
    ...
    return (
        <TextareaControl
            ...
            className="rrze-autoshare-twitter-tweet-text"
            maxLength={maxLength}
            label={
                <span
                    style={{ marginTop: "0.5rem", display: "block" }}
                    className="rrze-autoshare-twitter-prepublish__message-label"
                >
                    <span>
                        {__("Custom message:", "rrze-autoshare")}
                        &nbsp;
                    </span>
                    <span
                        id="rrze-autoshare-twitter-counter-wrap"
