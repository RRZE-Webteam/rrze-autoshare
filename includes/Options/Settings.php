...
class Settings
{
    ...
    public function styling()
    {
        ...
        echo '<style>.rrze-autoshare-error {color: #d63638; margin: 5px 0;}</style>';
    }
    ...
    {
        if (
            !isset($_POST['rrze_autoshare_settings_save'])
            || !wp_verify_nonce(
                $_POST['rrze_autoshare_settings_save'],
                'rrze_autoshare_settings_save_' . $this->optionName
            )
        ) {
            return;
        }
        ...
        if (!current_user_can($this->capability)) {
            wp_die(__('You do not have enough permissions to do that.', 'rrze-autoshare'));
        }

        $currentOptions = $this->getOptions();
        $submittedOptions = $_POST[$this->optionName] ?? [];
